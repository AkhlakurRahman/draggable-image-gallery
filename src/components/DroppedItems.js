...
import { DefaultImageOptions } from '../utils/imageControl';

const DroppedItems = ({ item, removeImageFromCanvas }) => {
  const [showModal, setShowModal] = useState(false);
  const [options, setOptions] = useState(DefaultImageOptions);
  ...
  const handleFilterChange = ({ target }) => {
    setOptions((prevOption) => {
      return prevOption.map((option, index) => {
        ...
        const modifiedImageOptions = { ...option, value: target.value };
        return modifiedImageOptions;
      });
    });
  };
  ...
  //   item: { type: ItemTypes.IMAGE, ...item, index },
  //   collect: (monitor) => ({
  ...
  return (
    <>
      <div className='dropped-item-container' key={item.char_id}>
        <img
          src={item.img}
          alt={item.name}
          style={selectedImageId === item.char_id ? getImageStyle() : null}
        />
        ...
        <div className='action-buttons'>
          ...
          <svg
            ...
            fill='none'
            xmlns='http://www.w3.org/2000/svg'
            onClick={() => removeImageFromCanvas(item.char_id)}
          >
            <path
